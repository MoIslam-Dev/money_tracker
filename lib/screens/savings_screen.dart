...
class SavingsGoalsScreen extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(g.name, style: t.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
                    if (g.targetDate != null)
                      Text('${strings.tr('target_date')}: ${dateKey(parseDateKey(g.targetDate!))}',
                          style: t.textTheme.labelSmall?.copyWith(color: t.colorScheme.onSurfaceVariant)),
                  ],
                ),
              ),
              PopupMenuButton<String>(
                onSelected: (v) => v == 'edit' ? _openEditor(context, g) : _delete(context, g),
                itemBuilder: (_) => [
          ...
          if (g.note != null && g.note!.isNotEmpty) ...[
            ...
            Text(g.note!, style: t.textTheme.bodySmall?.copyWith(color: t.colorScheme.onSurfaceVariant)),
          ],
          ...
          Row(
            children: [
              Expanded(
                child: Text('${formatDA(g.currentAmount)}  /  ${formatDA(g.targetAmount)}',
              ...
              if (reached)
                Container(
                  ...
                  child: Text(strings.tr('goal_reached'),
    ...
    await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(prefixText: 'DA '),
        ),
        actions: [
  ...
  Future<void> _openEditor(BuildContext context, [SavingsGoal? editing]) async {
    ...
    final targetCtrl = TextEditingController(text: editing == null ? '' : '${editing.targetAmount}');
    final currentCtrl = TextEditingController(text: editing == null ? '' : '${editing.currentAmount}');
    final noteCtrl = TextEditingController(text: editing?.note ?? '');
    DateTime? targetDate = editing?.targetDate == null ? null : parseDateKey(editing!.targetDate!);
    ...
    await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) {
          return AlertDialog(
            title: Text(editing == null ? strings.tr('add_savings') : strings.tr('edit_savings')),
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  TextField(
                      controller: nameCtrl,
                      decoration: InputDecoration(labelText: strings.tr('goal_name'))),
                  const SizedBox(height: 10),
                  TextField(
                      controller: targetCtrl,
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(labelText: strings.tr('target_amount'), prefixText: 'DA ')),
                  const SizedBox(height: 10),
                  TextField(
                      controller: currentCtrl,
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(labelText: strings.tr('current_amount'), prefixText: 'DA ')),
                  const SizedBox(height: 10),
                  InkWell(
                    borderRadius: BorderRadius.circular(8),
                    onTap: () async {
                    ...
                    child: InputDecorator(
                      ...
                      child: Text(targetDate == null ? '-' : dateKey(targetDate!)),
                    ),
                  ),
                  const SizedBox(height: 10),
                  TextField(
            ...
            actions: [
              ...
              FilledButton(
                onPressed: () async {
                  ...
                  if (target == null || nameCtrl.text.trim().isEmpty) return;
                  await state.saveGoal(
                    id: editing?.id,
                    name: nameCtrl.text.trim(),
                    target: target,
                    current: parseAmount(currentCtrl.text) ?? (editing?.currentAmount ?? 0),
  ...
  Future<void> _delete(BuildContext context, SavingsGoal g) async {
    ...
    if (ok == true && context.mounted) await context.read<AppState>().deleteGoal(g);
  }
}