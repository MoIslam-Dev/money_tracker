...
class BudgetsScreen extends StatelessWidget {
          ...
          child: ListView(
            ...
            children: [
              ...
              if (state.committedMonthly > 0)
                Text(
                  '${strings.tr('committed_monthly')}: ${formatDA(state.committedMonthly)} / ${strings.tr('per_month')}',
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    ...
                    color: totalBudget > 0 && totalSpent >= totalBudget
              ...
              Text('${totalBudget == 0 ? 0 : (totalSpent / totalBudget * 100).toStringAsFixed(0)}%',
                  style: t.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
            ],
          ),
          const SizedBox(height: 6),
          Text('${strings.tr('spent')}: ${formatDA(totalSpent)}',
              style: t.textTheme.labelSmall?.copyWith(color: t.colorScheme.onSurfaceVariant)),
        ],
      ),
    );
  ...
  Widget _budgetCard(BuildContext context, Budget b) {
    ...
    final spent = state.categorySpentInMonth(b.categoryId, state.currentMonth);
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(iconFor(cat?.icon ?? 'more_horiz'),
                    color: color, size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(cat == null ? strings.tr('other') : strings.categoryName(cat.name),
                        style: t.textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.w800)),
                    Text('${strings.tr('budget')}: ${formatDA(b.amount)}',
                        style: t.textTheme.labelSmall?.copyWith(color: t.colorScheme.onSurfaceVariant)),
                  ],
                ),
              ),
              PopupMenuButton<String>(
                onSelected: (v) => v == 'edit' ? _openEditor(context, b) : _delete(context, b),
                itemBuilder: (_) => [
          ...
          Row(
            children: [
              Expanded(
                child: LevelBar(value: pct / 100, color: color),
              ),
              const SizedBox(width: 12),
          ...
          Row(
            children: [
              Expanded(
                child: Text('${strings.tr('spent')}: ${formatDA(spent)}',
                    style: t.textTheme.labelSmall?.copyWith(color: t.colorScheme.onSurfaceVariant)),
              ),
              Text(
                pct >= 100
                    ? '${strings.tr('over_budget')} ${formatDA(-remaining)}'
                    : '${strings.tr('remaining')}: ${formatDA(remaining)}',
                style: TextStyle(
                  color: pct >= 100
                      ? AppColors.expenseOn(context)
    ...
    await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) {
          ...
          return AlertDialog(
            title: Text(editing == null ? strings.tr('add_budget') : strings.tr('edit_budget')),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(strings.tr('budget_amount'), style: t.textTheme.labelMedium),
                  const SizedBox(height: 8),
                  TextField(
                    controller: amountCtrl,
                    keyboardType: TextInputType.number,
                    decoration: InputDecoration(prefixText: 'DA '),
                  ),
                  const SizedBox(height: 16),
                  if (cats.isEmpty)
                    Text(strings.tr('no_income'), style: t.textTheme.bodySmall)
                  else
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: [
                        for (final c in cats)
                          ChoiceChip(
                            label: Text(strings.categoryName(c.name)),
            ...
            actions: [
              ...
              FilledButton(
                onPressed: () async {
                  ...
                  await state.saveBudget(id: editing?.id, categoryId: categoryId!, amount: amount);
...
Future<void> _delete(BuildContext context, Budget b) async {
  ...
  if (ok == true && context.mounted) await context.read<AppState>().deleteBudget(b);
}
}