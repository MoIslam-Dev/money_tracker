...
class SettingsStore {
  ...
  Future<void> load() async {
    ...
    if (rawSlots != null && rawSlots.isNotEmpty) {
      try {
        final list =
            (jsonDecode(rawSlots) as List).cast<Map<String, dynamic>>();
  ...
  Future<void> save() async {
    ...
    await p.setString(
      _kNotifSlots,
      jsonEncode(notifSlots.map((s) => s.toJson()).toList()),
    );
    if (pinHash != null) {
  ...
  Future<void> addCustomCurrency(AppCurrency c) async {
    customCurrencies.removeWhere((x) => x.code == c.code);
    customCurrencies.add(c);
    await save();
  }

  List<AppCurrency> _loadCustomCurrencies(String? raw) {
    if (raw == null || raw.isEmpty) return [];
    try {
      final list = (jsonDecode(raw) as List).cast<Map<String, dynamic>>();
      return list.map(AppCurrency.fromJson).toList();
    } catch (_) {
      return [];
    }
  }
