...
Border _border([String hex = _borderHex]) => Border(
  borderStyle: BorderStyle.Thin,
  borderColorHex: ExcelColor.fromHexString(hex),
);
...
NumFormat _moneyFormatFor(String? code, String fallback) => NumFormat.custom(
  formatCode:
      '#,##0 "${currency_model.resolveCurrency(code ?? fallback).symbol}"',
);
...
Uint8List buildTransactionsWorkbook(
  ...
  for (var i = 0; i < txns.length; i++) {
    ...
    final category =
        cat == null ? sheets.tr('other') : state.categoryLabel(cat);
    ...
    put(
      0,
      DateCellValue.fromDateTime(parseDateKey(t.date)),
      _bodyStyle(
        alt: alt,
        align: HorizontalAlign.Center,
        numFormat: _dateFormat,
      ),
    );
    put(
      1,
      TextCellValue(sheets.tr(t.isExpense ? 'expense' : 'income')),
    ...
    put(
      ...
      _bodyStyle(
        ...
        bold: true,
        numFormat: _moneyFormatFor(t.currency, state.currency),
      ),
    );
    put(
      5,
      TextCellValue(sheets.tr(t.paymentMethod)),
      _bodyStyle(alt: alt, color: _mutedHex),
    );
  }
...
void _buildSummary(
  ...
  for (final t in txns) {
    ...
    final cat = state.categoryById(t.categoryId);
    final label = cat == null ? sheets.tr('other') : state.categoryLabel(cat);
  ...
  void pair(
    ...
    v.value = value;
    v.cellStyle = _bodyStyle(
      bold: true,
      color: color ?? _bodyHex,
      align: HorizontalAlign.Right,
    );
  }
...
Uint8List _freezeAndFilter(
  ...
  try {
    ...
    final sheetViews =
        worksheet.getElement('sheetViews') ?? XmlElement(XmlName('sheetViews'));
...
Future<String> exportExcel(
  ...
  final file = File(
    '${dir.path}/money_tracker_${dateKey(DateTime.now())}.xlsx',
  );
